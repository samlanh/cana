...
{        
     ...
     * */
	  public function showCopyBuntton($url_copy) {
    	...
    	$copyButton = '&nbsp;<a href="#" class="btn btn-sm green  pull-right" onClick="copyRecord(\''.$url_copy.'\')">'
    			.'<b>'
    					.$tr->translate("COPY")
    					.'</b></a>';
    	return $copyButton;
    ...
    public function getCheckList($delete=0, $columns,$rows,$link=null,$editLink="", $class='items', $textalign= "left", $report=false, $id = "table")
    {
    	...
    	$col_str .='<thead><tr>';
    	if($delete== 1) {
    		$col_str .= '<th class="tdheader tdcheck" style="width:12px !important"></td>';
    	}
    				...
    				$i=0;
		  			foreach($row as $key=>$read) {
		  				...
		  				if($i==0) {
		  					$temp=$read;
		  					if($delete==1){
				    			$row_str .= '<td style="width:12px !important"><input type="checkbox" name="del[]" id="del[]" value="'.$temp.'" /></td>';
		  					}
