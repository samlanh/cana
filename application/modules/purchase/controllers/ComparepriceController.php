<?php
class Purchase_ComparepriceController extends Zend_Controller_Action
{	
	...
	public function addAction(){
		...
		if($this->getRequest()->isPost()) {
			...
			$data["id"] = $id;
			try {
				$payment_purchase_order = new Purchase_Model_DbTable_DbPriceCompare();
				$ids=$payment_purchase_order->add($data);
				Application_Form_FrmMessage::message("Price Compare has been Saved!"); 		
				if(isset($data["save_print"])){
					Application_Form_FrmMessage::redirectUrl("/purchase/compareprice/purproductdetail/id/".$ids);
				}else{
					Application_Form_FrmMessage::redirectUrl("/purchase/compareprice/addcompare");
				}
			}catch (Exception $e){
		...
		$rs =  $db->getProductRequestDetail($id);
		$this->view->product = $db->getProductRequestDetail($id);
		
		$this->view->code = $db->getPriceCode($rs[0]["branch_id"]);
	...
	public function editAction(){
		...
		if($this->getRequest()->isPost()) {
			...
			try {
				$payment_purchase_order = new Purchase_Model_DbTable_DbPriceCompare();
				$ids=$payment_purchase_order->edit($data);
				Application_Form_FrmMessage::message("Price Compare has been Saved!"); 		
				if(isset($data["save_print"])){
					Application_Form_FrmMessage::redirectUrl("/purchase/compareprice/purproductdetail/id/".$ids);
				}else{
