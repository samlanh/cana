...
{	
	...
	function getAllPurchaseOrder($search){//new
		...
		$sql=" SELECT id,
			...
			(SELECT p.`re_code` FROM `tb_purchase_request` AS p WHERE p.id=re_id LIMIT 1) AS re_code,
			(SELECT p.`date_request` FROM `tb_purchase_request` AS p WHERE p.id=re_id LIMIT 1) AS date_request,
			(SELECT `name` FROM `tb_plan` AS pl WHERE pl.id=(SELECT p.`plan_id` FROM `tb_purchase_request` AS p WHERE p.id=re_id LIMIT 1 )) AS plan,
		...
		if(!empty($search['text_search'])){
			...
			$s_where[] = " (SELECT p.`re_code` FROM `tb_purchase_request` AS p WHERE p.id=re_id LIMIT 1 ) LIKE '%{$s_search}%'";
