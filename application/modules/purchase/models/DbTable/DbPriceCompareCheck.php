...
class Purchase_Model_DbTable_DbPriceCompareCheck extends Zend_Db_Table_Abstract
{
	...
	function getProduct($id){
		...
		$sql = "SELECT 
				  ...
				  (SELECT u.fullname FROM tb_acl_user as u WHERE u.user_id=(SELECT s.user_id FROM `tb_price_compare_remark` AS s WHERE s.re_id=pc.`re_id` LIMIT 1) LIMIT 1) AS appr_name,
	...
	function add($data){
		...
		try{
			if($data["approve"]==1){
				...
				$is_approve =0;
				$is_edit = 0;
				$reject_count = $data["check_reject_count"];
			}else{
				$pedding = 4;
				$appr_status=2;
				$is_approve =0;
				$is_edit = 1;
				$reject_count = $data["check_reject_count"]+1;
			}
				$arr = array(
					...
					'is_edit'				=>	$is_edit,
					'check_reject_count'	=>	$reject_count,
				);
			...
			$rs_reject = $db_globle->getPriceCompareRejectExist($data["id"],1);
				if(!empty($rs_reject)){
					$arr_remark = array(
						'user_id'	=>	$GetUserId,
						'date'		=>  date('Y-m-d'),
