<?php
class Sales_QuotecheckController extends Zend_Controller_Action
{	
   	...
   	public function indexAction()
	{
		...
		$db = new Sales_Model_DbTable_Dbquotecheck();
		$rows = $db->getAllSaleOrder($search);
		$this->view->rs = $rows;
		$columns=array("BRANCH_NAME","CUSTOMER_NAME","REQUEST_NO", "REQUEST_DATE","TOTAL","TOTAL_AMOUNT","APPROVED_STATUS","PENDING_STATUS","BY_USER");
		$link=array('module'=>'sales','controller'=>'quotecheck','action'=>'add');
		$link1=array('module'=>'sales','controller'=>'quotecheck','action'=>'edit');
	...
	function approvedAction(){
		...
		if($this->getRequest()->isPost()) {
			...
			try {
				$dbq = new Sales_Model_DbTable_Dbquotecheck();				
				$dbq->addQuoateOrderCheck($data);
				Application_Form_FrmMessage::message("APPROVED_SUCESS");
				Application_Form_FrmMessage::redirectUrl("/sales/quotecheck/index");
			}catch (Exception $e){
				$err =$e->getMessage();
				Application_Model_DbTable_DbUserLog::writeMessageError($err);
				Application_Form_FrmMessage::Sucessfull("APPROVED_FAIL", "/sales/quotecheck");
			}
		}
		Application_Form_FrmMessage::message("APPROVED_SUCESS");
		Application_Form_FrmMessage::redirectUrl("/sales/quotecheck/index");
	...
	function addAction(){
    	...
    	if(empty($id)){
    		$this->_redirect("/sales/quotecheck");
    	}
    	
    	$query = new Sales_Model_DbTable_Dbquotecheck();
