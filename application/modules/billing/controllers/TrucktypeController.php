<?php
class Billing_TrucktypeController extends Zend_Controller_Action
{
...
//     				'end_date'=>date("Y-m-d"),
    	...
    	$db = new Billing_Model_DbTable_DbTrucktype();
    	$db_glob=new Application_Model_GlobalClass();
    	$rows = $db->getAllTruckType($search);
    	$rows=$db_glob->getImgStatus($rows, BASE_URL);
    	$this->view->rs = $rows;
    	$list = new Application_Form_Frmlist();
    	$columns=array("TRUCT_CAT_NAME_EN","TRUCT_CAT_NAME_EN","DATE","STATUS");
    	$link=array(
    			'module'=>'billing','controller'=>'trucktype','action'=>'edit',
    	);
	...
	public function addAction()
	{
		if($this->getRequest()->isPost()) {
			...
			if(isset($data['saveclose'])){
				...
				Application_Form_FrmMessage::redirectUrl('/billing/trucktype');
			}
			else{
				Application_Form_FrmMessage::message("INSERT_SUCCESS");
				Application_Form_FrmMessage::redirectUrl('/billing/trucktype/add');
			}
		}
		$formFilter = new Billing_Form_FrmTrucktype();
		$formAdd = $formFilter->frm_truck_type();
	...
	public function editAction()
	{
		...
		$formFilter = new Billing_Form_FrmTrucktype();
		$formAdd = $formFilter->frm_truck_type($row);
		$this->view->frmAdd = $formAdd;
		Application_Model_Decorator::removeAllDecorator($formAdd);
