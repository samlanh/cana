...
class Product_Model_DbTable_DbProduct extends Zend_Db_Table_Abstract
{
    ...
    function updatecode(){
		$db =$this->getAdapter();
		$sql = "SELECT c.id,c.`name`,c.start_code,c.`prefix` FROM `tb_category` AS c WHERE c.`parent_id`=0";
		$parent = $db->fetchAll($sql);
		//print_r($parent);exit();
		
		foreach($parent as $rs){$j=0;
			$start_code = $rs['start_code'];
			$end_code = ($rs['start_code']+1000)-1;
			
			
			$sql = "SELECT c.id,c.`name`,c.`prefix`,c.start_code FROM `tb_category` AS c WHERE c.`parent_id`=".$rs["id"];
			$sub = $db->fetchAll($sql);
			
			foreach($sub as $rss){ 
				//$sqlup ="UPDATE tb_category set start_code='".$start_code."',end_code='".$end_code."' WHERE id=".$rss["id"];
...
// $db->getProfiler()->setEnabled(false);
			...
			$start_code = $start_code+1000;
			$end_code = $end_code + 1000;
			}
			
			$sql = "SELECT c.id,c.`name`,c.`prefix`,c.start_code FROM `tb_category` AS c WHERE c.`parent_id`=".$rs["id"];
			$sub = $db->fetchAll($sql);
			foreach($sub as $rss){ $i=0;
				$sql = "SELECT id FROM `tb_product` AS p WHERE p.`cate_id`=".$rss["id"];
				$pro = $db->fetchAll($sql);
				foreach($pro as $rsp){$i++;
					$sql ="UPDATE tb_product SET int_code='".($rss["start_code"]+$i)."' WHERE id=".$rsp["id"];
					$db->getProfiler()->setEnabled(true);
					$db->query($sql);
					Zend_Debug::dump($db->getProfiler()->getLastQueryProfile()->getQuery());
Zend_Debug::dump($db->getProfiler()->getLastQueryProfile()->getQueryParams());
$db->getProfiler()->setEnabled(false);
				...
				$sql = "SELECT id FROM `tb_product` AS p WHERE p.`cate_id`=".$rs["id"];
				$pros = $db->fetchAll($sql);
				foreach($pros as $rsp){$j++;
					$sql ="UPDATE tb_product SET int_code='".($rss["start_code"]+$i)."' WHERE id=".$rsp["id"];
					$db->getProfiler()->setEnabled(true);
					$db->query($sql);
					Zend_Debug::dump($db->getProfiler()->getLastQueryProfile()->getQuery());
...
function checkCateparent($id){
  ...
  public function deleteProduct($id){
		...
		$db->delete('tb_product', $where);
		$where = $db->quoteInto('pro_id = ?', $id);
		$db->delete('tb_prolocation', $where);
  }
